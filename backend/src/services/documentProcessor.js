...
class DocumentProcessor {
  ...
  async processPDF(filePath) {
    ...
    try {
      ...
      console.warn('[DocumentProcessor] pdf-parse not available, using placeholder');
      return `Content from PDF file: ${path.basename(filePath)} - PDF processing not available in demo. Please use text or DOCX files for full functionality.`;
    }
    ...
    try {
      ...
      if (!data.text || data.text.trim().length === 0) {
        return `Content from PDF file: ${path.basename(filePath)} - PDF appears to be empty or contains only images.`;
      }

      return data.text.trim();
    } catch (error) {
      console.error('[DocumentProcessor] PDF parse error:', error);
      return `Content from PDF file: ${path.basename(filePath)} - Failed to parse PDF: ${error.message}. Make sure the PDF contains readable text.`;
    }
  }
  ...
  async processTXT(filePath) {
    try {
      ...
      return `Error reading text file: ${error.message}. Please ensure the file is a valid UTF-8 text file.`;
    }
  }
